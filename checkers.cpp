...
vec2 quad_texCoord[6] = { // draw a square quad
 // upper half
  vec2(0.0, 0.0),  // for a
  vec2(0.0, 1.0),  // for b
  vec2(1.0, 1.0),  // for c
...
void makeCheckImage(void)
{
   ...
   for (i = 0; i < checkImageHeight; i++) {
	...
	c = (((i & 0x8) == 0) ^ ((j & 0x8) ==0)); // i&8 ==8 is to split the grid into 8's , and if i and j are the same (0,0) or (1,1), then assign the same texture color
...
void init(void)
{
   ...
   /*
   suppose we want 10 objs , then we use 
   GLUint texName[10];
   glGenTextures(10,texName);
   then we use texName[0], texName[1]... to use the texture objs
   ...
   */
   ...
   glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, checkImageWidth, checkImageHeight, 
                0, GL_RGBA, GL_UNSIGNED_BYTE, checkImage); //first 0 means the levels of details = 0,
